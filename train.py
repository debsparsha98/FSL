...
if __name__ == '__main__':
    ...
    ap.add_argument("-w", "--weight", type=str, default='D:/datasets/visdrone/yolo_nas_l_coco.pth',
    ...
    # Get number of classes from the COCO JSON file
    json_annotation_path = os.path.join(yaml_params['Dir'], yaml_params['labels']['train'])
    with open(json_annotation_path, 'r') as f:
        coco_data = json.load(f)
        no_class = len(coco_data['categories'])
    ...
    test_loader = None
    if 'test' in yaml_params['images'] and 'test' in yaml_params['labels']:
    ...
    if not os.path.exists(weight_path):
        ...
        print("[INFO] Please make sure the yolo_nas_l_coco.pth file is in D:/datasets/visdrone/")
        exit(1)

    # Initialize model with local weights
    if args['resume'] or weight_path.endswith('.pth'):
        print(f"[INFO] Loading model from local weights: {weight_path}")
        model = models.get(
            args['model'],
        ...
        # This should not happen since we set default to local path
        print(f"[ERROR] Invalid weight path: {weight_path}")
        exit(1)
    ...
    for key, value in train_params.items():
        if key not in ['loss', 'valid_metrics_list']:  # Skip complex objects
            print(f"  {key}: {value}")
    ...
    # Load best model
    best_model_path = os.path.join('runs', name, 'ckpt_best.pth')
    if os.path.exists(best_model_path):
        ...
        print('\033[1m[INFO] Validating Best Model:\033[0m')
        ...
        # Evaluating on Test Dataset if available
        if test_loader is not None:
            print('\033[1m[INFO] Testing Best Model:\033[0m')
    ...
    print(f'[INFO] Training Completed in \033[1m{(time.time()-s_time)/3600:.2f} Hours\033[0m')